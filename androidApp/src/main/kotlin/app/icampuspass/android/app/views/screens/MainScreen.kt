...
import app.icampuspass.android.app.models.StringFormatter
                                    ...
                                    Column(
                                        ...
                                        Text(
                                            text = StringFormatter
                                                .formatCourseNameZhHant(
                                                    name = obj.get(key = "ch_cos_name").toString(),
                                                    note = obj.get(key = "note").toString()
                                                ),
                                        ...
                                        Text(
                                            text = StringFormatter
                                                .formatCourseNameEn(
                                                    name = obj.get(key = "en_cos_name").toString()
                                                ),
                                            fontWeight = FontWeight.SemiBold
                                        )
                                        ...
                                        Text(
                                            text = StringFormatter
                                                .formatInstructorNameEn(
                                                    name = obj.get(key = "teach_name_en").toString()
                                                )
                                        )

                                        val location = obj.get(key = "room").toString()
                                            .removeSurrounding("\"").trim()

                                        val gradeNumber = obj.get(key = "seatno").toString()
                                            .removeSurrounding("\"").trim()

                                        if (location.isNotBlank() || gradeNumber.isNotBlank()) {
                                            Spacer(modifier = Modifier.padding(vertical = 4.dp))

                                            Text(
                                                text = when {
                                                    gradeNumber.isBlank() -> "地點：${location}"
                                                    location.isBlank() -> "成績座號：${gradeNumber}"
                                                    else -> "地點：${location}　成績座號：${gradeNumber}"
                                                }
                                            )
                                        }

                                        val courseNote = StringFormatter
                                            .formatCourseNoteZhHant(
                                                note = obj.get(key = "note").toString()
                                            )

                                        if (courseNote.isNotBlank()) {
                                            Spacer(modifier = Modifier.padding(vertical = 4.dp))
